...
class BERT_HLSQG(object):
    def __init__(self):
        ...
        modelpath = 'model/SQG_hl_id3_bert-base-uncased'
        training_modelpath = 'model/ENG_HLSQG_model/pytorch_model_2.bin'
    ...
    def predict(self, context, answer, answer_start, BS = 5):
        ...
        for BS_size in range(BS, BS+1):
            ...
            # gen_QG.append(sorted(tmp_QG, key=lambda x:x[0], reverse=True)[0])

        result = []
        
        for ele in sorted(gen_QG, key=lambda x:x[0], reverse=True):
            Q = ele[1][2].replace('[SEP]','').replace('？','?')
            # if answer.lower() not in ele[1][2]:
            if Q not in result:
                result.append(Q)
        
        return result

    def predict2(self, context, answer, answer_start):
        rouge = Rouge()
        data = self.read_data(context = context, answer = answer, answer_start = answer_start)
        ...
        for BS_size in range(1, self.BS_size+1):
            input_ids = torch.tensor([f.input_ids for f in features], dtype=torch.long)
            input_mask = torch.tensor([f.input_mask for f in features], dtype=torch.long)
            segment_ids = torch.tensor([f.segment_ids for f in features], dtype=torch.long)
        ...
        for ele in sorted(gen_QG, key=lambda x:x[0], reverse=True):
            ...
            if len(result) == 0:
                ...
                for al_Q in result:
                    ...
                    if rouge_scores['rouge-l']['f'] > 0.9:
                        flag = 1
                        break
                if flag == 0:
                    result.append(Q)

        result = [ele for ele in result if answer not in ele]
        return result
